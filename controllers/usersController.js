...
const createNewUser = asyncHandler(async (req, res) => {
    ...
    const userObj = {userName: userName, password:hashedPassword }
...
const updateUser = asyncHandler(async (req, res) => {
    const {id, userName, password, images, players} = req.body
    // check data
    if(!id || !userName){
        return res.status(400).json({Message: 'all fields are required' })
    }
    ...
    if(duplicate && duplicate?._id.toString() !== id){
        return res.status(409).json({Message: 'duplicate username. please choose a different one'})
    }
